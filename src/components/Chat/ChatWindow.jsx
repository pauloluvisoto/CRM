...
const ChatWindow = ({ conversation, lastSelectedAt, onMessageSent }) => {
    ...
    const handleSend = async (e, type = 'text', contentOverride = null) => {
        ...
        try {
            // 1. Save to Database directly to ensure media URL is preserved
            const { data: savedMsg, error: dbError } = await supabase
                .from('social_messages')
                .insert([{
                    conversation_id: conversation.id,
                    content: type === 'text' ? finalContent : (msgPreview.replace(prefix, '') || 'Mídia'),
                    direction: 'outbound',
                    status: 'sent',
                    media_url: type !== 'text' ? finalContent : null,
            ...
            if (dbError) {
                ...
                throw dbError; // Stop if DB save fails
            }
            ...
            if (conversation.platform === 'whatsapp') {
                ...
                if (type === 'text') {
                    response = await wApi.sendText(phone, finalContent, replyToId);
                } else if (type === 'audio') {
                    response = await wApi.sendAudio(phone, finalContent, replyToId);
                } else if (type === 'image') {
                    response = await wApi.sendImage(phone, finalContent, '', replyToId);
                } else if (type === 'video') {
                    response = await wApi.sendVideo(phone, finalContent, '', replyToId);
                } else if (type === 'document') {
                    response = await wApi.sendDocument(phone, finalContent, metadata.filename || 'document', replyToId);
                } else if (type === 'sticker') {
                    response = await wApi.sendSticker(phone, finalContent, replyToId);
                } else if (type === 'gif') {
                    response = await wApi.sendGif(phone, finalContent, replyToId);
                }

                // 4. Update External ID if available
                if (response && (response.messageId || response.id)) {
                    const wapiMessageId = response.messageId || response.id;
                    await supabase
                        .from('social_messages')
                        .update({ external_id: wapiMessageId })
                        .eq('id', savedMsg.id);
                }
            } else {
            ...
            // If we have a saved message, mark it as failed
            // Note: savedMsg is defined in the try block, so we might not have access to it here easily without moving it up scope
